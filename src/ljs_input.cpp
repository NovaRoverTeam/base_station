...
{
  ros::init(argc, argv, "ljs_input");
  ros::NodeHandle n;
  ros::Rate loop_rate(LOOP_HZ);

  ros::Publisher raw_ctrl_pub = 
    n.advertise<nova_common::RawCtrl>("/base_station/ljs_raw_ctrl", 1);
  ...
  while (ros::ok())
  {
    ...
    msg.but_b_trg = GamepadButtonDown(controller, BUTTON_B);
    ...
    msg.trig_l_val = 0.0;
      ...
      if (fabsf(msg.trig_l_val) < 0.01) { // Get rid of tiny floats
        msg.trig_l_val = 0.0;
      }
      twist_lock = false;
    }
